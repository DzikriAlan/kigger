...
const insightImages: Record<string, { imageUrl: string; imageAlt: string; grayscaleImage?: boolean }> = {
...
export default function InsightsSection() {
  ...
  const insights = Object.entries(t.insights.items).map(([id, insight]) => ({
    id,
    ...insight,
    ...insightImages[id],
  }));
  ...
  return (
    <section id="insights" className="border-t border-divider bg-midnight py-20 lg:py-28">
      <div className="px-6 sm:px-14 lg:px-24 xl:px-40 2xl:px-52">
        ...
        <p className="mt-6 max-w-xl text-base text-white">{t.insights.description}</p>
      </div>
